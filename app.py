...
if menu == "Dataset":
    ...
    axes[0, 0].set_title("Distribusi Status Nasabah", fontsize=13, fontweight='bold', color=NAVY, pad=12)
    legend_elements = [
    ...
    axes[0, 1].set_title("Distribusi Umur Nasabah", fontsize=13, fontweight='bold')
    ...
    axes[1, 0].set_title("Distribusi Gender", fontsize=13, fontweight='bold')

    # Bar H: Pendidikan
    dist_edu = df['Education_Level'].value_counts()
    bar_colors = [NAVY, ROYAL, "#2980B9", "#5BA4CF", "#8FADD4", GOLD, AMBER][:len(dist_edu)]
    axes[1, 1].barh(
        dist_edu.index, dist_edu.values,
        color=bar_colors, edgecolor='white', height=0.65
    )
    axes[1, 1].set_title("Distribusi Tingkat Pendidikan", fontsize=13, fontweight='bold')
    axes[1, 1].set_xlabel("Jumlah Nasabah")
    axes[1, 1].set_ylabel("Pendidikan")
    for i, v in enumerate(dist_edu.values):
        axes[1, 1].text(v + 20, i, str(v), va='center', fontsize=9, color=NAVY)
...
# ============================================================
    ...
    if "df_result" in st.session_state:
        ...
        for i, (bar, score) in enumerate(zip(bars, top_data["Skor_SAW"])):
            medal = ["🥇", "🥈", "🥉"][i] if i < 3 else ""
...
# ============================================================
    ...
    if 'df_result' in st.session_state:
        ...
        st.markdown('<span class="section-badge">SAW Distribution</span>', unsafe_allow_html=True)
        ...
        # Scatter rank vs score — warna per tier
        total = len(df_result)
        rank_colors = [
            GOLD   if r <= 3               else
            AMBER  if r <= int(total*0.10) else
            ROYAL  if r <= int(total*0.50) else CRIMSON
